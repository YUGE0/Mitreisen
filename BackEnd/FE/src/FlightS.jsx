...
export default function FlightS(prop){

    const navigate = useNavigate();
    //const location = useLocation();

    // let df = prop.info.desFrom
    ...
    function handelSearch(){
        //setFlightData({desF:desFrom,desT:desTo,trv:traveler,daF:dateFrom,daT:dateTo,cl:cla,})
        //setFlightData(desFrom,desTo,traveler,dateFrom,dateTo,cla);
        navigate("/FlightB",{state:{desFrom,desTo,traveler,dateFrom,dateTo,cla,trip}})
          
    }
    //console.log("it is Prop"+prop.info)


    return(
        <>
        <div className="p-8 px-20 h-fit text-xl rounded-xl bg-Scolor text-Fcolor">
        <h1 className="text-2xl font-medium">Search Flight</h1>
        <form>
        <div className="flex flex-wrap flex-row justify-evenly items-center">
                <div className="border-2 rounded-xl mt-2 p-10 h-fit border-Fcolor flex flex-wrap justify-between items-center">
                    <div>
                        ...
                        <input value={desFrom} onChange={(e)=> setDesFrom(e.target.value)} className="bg-Scolor p-2 w-52" placeholder="your departure?"/>
                    </div>
                    <div>
                ...
                <div className="border-2 rounded-xl mt-2 p-10 h-fit border-Fcolor flex flex-wrap justify-between items-center">
                    ...
                    <div>
                        ...
                        <select value={cla} name="cl" onChange={(e)=> setCla(e.target.value)} className=" bg-transparent text-Tcolor">
                            ...
                            <option className=" bg-Fcolor text-Scolor">Premium Economy</option>
                        </select>
                    </div>
                </div>
        </div>
        ...
        <div className="flex flex-wrap gap-5">
            ...
            <div>
                <input name="trip" value={trip} onClick={()=>setTrip("One")} type="radio"/> One Way
            </div>
        </div>
        </div>
        </div>
